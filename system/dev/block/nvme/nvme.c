...
static bool io_process_txn(nvme_device_t* nvme, nvme_txn_t* txn) {
    zx_handle_t vmo = txn->op.u.rw.vmo;
    nvme_utxn_t* utxn;
    zx_status_t r;
    ...
    for (;;) {
        ...
        // max transfer size
        uint32_t blocks = xfer / nvme->info.block_size;

        if ((r = zx_vmo_op_range(vmo, ZX_VMO_OP_COMMIT,
                                 txn->op.u.rw.offset_vmo, xfer, NULL, 0)) != ZX_OK) {
        ...
        if ((r = zx_vmo_op_range(vmo, ZX_VMO_OP_LOOKUP,
                                 txn->op.u.rw.offset_vmo, xfer, pages, PAGE_SIZE)) != ZX_OK) {
        ...
        size_t pagecount = ((txn->op.u.rw.offset_vmo & PAGE_MASK) + xfer + PAGE_MASK) / PAGE_SIZE;

        nvme_cmd_t cmd;
        memset(&cmd, 0, sizeof(cmd));
        cmd.cmd = NVME_CMD_CID(utxn->id) | NVME_CMD_PRP | NVME_CMD_NORMAL | NVME_CMD_OPC(txn->opcode);
        cmd.nsid = 1;
        cmd.u.rw.start_lba = txn->op.u.rw.offset_dev;
        ...
        cmd.dptr.prp[0] = pages[0] | (txn->op.u.rw.offset_vmo & PAGE_MASK);
        if (pagecount == 2) {
        ...
        txn->op.u.rw.offset_dev += blocks;
        txn->op.u.rw.offset_vmo += xfer;
        txn->remain -= xfer;
        txn->pending_utxns++;
...
static void io_process_cpls(nvme_device_t* nvme) {
    ...
    while (nvme_io_cq_get(nvme, &cpl) == ZX_OK) {
        ...
        txn->pending_utxns--;
        if ((txn->pending_utxns == 0) && (txn->remain == 0)) {
...
static void nvme_queue(void* ctx, block_op_t* op) {
    ...
    switch (txn->op.u.command & BLOCK_OP_MASK) {
    case BLOCK_OP_READ:
    ...
    // scale vmo block offset to byte offset
    // scale blocks to transfer to bytes remaining
    txn->op.u.rw.offset_vmo *= nvme->info.block_size;
    txn->remain = (txn->op.u.rw.length + 1U) * nvme->info.block_size;
    txn->pending_utxns = 0;
    txn->flags = 0;

    zxlogf(SPEW, "nvme: io: %s: %ublks @ blk#%zu\n",
           txn->opcode == NVME_OP_WRITE ? "wr" : "rd",
           txn->op.u.rw.length + 1U, txn->op.u.rw.offset_dev);

    STAT_INC(total_ops);
    STAT_ADD(total_bytes, txn->remain);
